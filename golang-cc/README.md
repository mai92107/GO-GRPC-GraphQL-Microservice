...
Gin、PostgreSQL 與 React 建置的信用卡推薦產品。管理員維護銀行、卡片、回饋方案、消費類別與回饋單位；會員管理卡片夾、偏好、推薦與交易。
...
卡片可建立多期不重疊回饋方案，系統依交易日期選擇當期方案。方案內可建立多個優惠條件：

- 同一累計層只採回饋價值最高的條件，不同累計層可相加。
- 條件可設定個別月上限。
- 回饋方案必須同時符合消費類別與標準店家代碼；未設定店家代表所有店家適用。選擇「其他」時店家名稱僅供紀錄。
