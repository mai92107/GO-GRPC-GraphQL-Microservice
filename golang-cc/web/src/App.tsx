...
export default function App() {
  const [user, setUser] = useState<User | null>(null),
    ...
    [page, setPage] = useState<Page>("recommend");
  useEffect(() => {
    api<{ user: User; csrf_token: string }>("/public/auth/me")
      .then((x) => {
        setUser(x.user);
  ...
  if (user.role === "admin")
    return (
      <AdminApp
        user={user}
        onLogout={() =>
          mutate("/public/auth/logout", "POST").finally(() => setUser(null))
        }
      />
    );
  return (
...
function Auth({ onLogin }: { onLogin: (user: User, csrf: string) => void }) {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      if (mode === "accept") {
        await mutate("/public/auth/accept-invitation", "POST", {
          token,
          display_name: name,
          password,
        });
        ...
        await mutate("/public/auth/reset-password", "POST", {
          token,
          password,
        });
        ...
        const data = await mutate<{ user: User; csrf_token: string }>(
          "/public/auth/login",
          "POST",
          { email, password },
        );
  ...
  async function reset() {
    setError("");
    await mutate("/public/auth/request-password-reset", "POST", { email });
...
function Recommend() {
  const [amount, setAmount] = useState("200"),
    ...
    [availableCategories, setAvailableCategories] = useState<
    ...
    [notice, setNotice] = useState("");
  useEffect(() => {
    api<{ code: string; name: string }[]>("/member/categories")
      .then((categories) => {
  ...
  useEffect(() => {
    if (!category) return;
    api<Merchant[]>(
      `/member/merchants?category_code=${encodeURIComponent(category)}`,
    )
      .then((stores) => {
        setMerchants(stores);
        setMerchantCode("");
  ...
  const merchantName = merchantCode
    ? merchants.find((x) => x.code === merchantCode)?.name || ""
    : otherMerchant;
  async function run(e?: FormEvent) {
    e?.preventDefault();
    setError("");
    try {
      const data = await mutate<{ recommendations: Recommendation[] }>(
  ...
  async function transact(
    ...
    try {
      const data = await mutate<{ recommendation_changed: boolean }>(
        "/member/transactions",
        "POST",
        {
          ...
          recommendation_summary: option.allocations.map((a) => ({
            rule_id: a.rule_id,
            allocated_reward: a.allocated_reward,
          })),
        },
      );
      setNotice(
  ...
  return (
    <>
        ...
        <section className="results" aria-live="polite">
          {results.length === 0 ? (
            <div className="panel">
              <Empty
                title="準備好開始推薦"
                text="填入左側消費資訊，我們會計算每張卡與支付方式的有效回饋。"
              />
            </div>
          ) : (
            results.map((card) => (
      ...
      {confirm && (
        <Dialog title="確認刷卡" onClose={() => setConfirm(null)}>
          <div className="stack">
            <p>
              將以 <strong>{confirm.card.card_name}</strong> 建立在「
              {merchantName || "其他店家"}」的 NT${amount} 交易。
            </p>
            {confirm.option.payment_methods.length > 1 && (
              <Field label="實際支付方式">
                <select
                  value={confirm.paymentCode}
                  onChange={(e) =>
                    setConfirm({ ...confirm, paymentCode: e.target.value })
                  }
                >
                  {confirm.option.payment_methods.map((method) => (
            ...
            {confirm.option.reminders.map((x) => (
              <div className="notice" key={x}>
                {x}
              </div>
            ))}
            <button
              className="button"
              onClick={() =>
                transact(confirm.card, confirm.option, confirm.paymentCode)
              }
            >
              確認並記錄交易
            </button>
...
function Cards() {
  const [cards, setCards] = useState<Card[]>([]),
    [catalog, setCatalog] = useState<import("./api").CatalogCard[]>([]),
    [open, setOpen] = useState(false),
    [detail, setDetail] = useState<import("./api").CatalogCard | null>(null),
    [units, setUnits] = useState<import("./api").Unit[]>([]),
    [categories, setCategories] = useState<{ code: string; name: string }[]>(
      [],
    ),
    [methods, setMethods] = useState<import("./api").PaymentMethod[]>([]),
    [merchants, setMerchants] = useState<Merchant[]>([]),
  ...
  const load = () =>
    Promise.all([
      api<Card[]>("/member/cards"),
      api<import("./api").CatalogCard[]>("/member/catalog/cards"),
      api<import("./api").Unit[]>("/member/reward-units"),
      api<{ code: string; name: string }[]>("/member/categories"),
      api<import("./api").PaymentMethod[]>("/member/payment-methods"),
    ]).then(([c, k, u, cs, p]) => {
      setCards(c);
      setCatalog(k);
      setUnits(u);
      setCategories(cs);
      setMethods(p);
      if (!form.card_product_id && k[0])
        setForm((v) => ({ ...v, card_product_id: k[0].id }));
      return Promise.all(
        cs.map((x) =>
          api<Merchant[]>(
            `/member/merchants?category_code=${encodeURIComponent(x.code)}`,
          ),
        ),
      ).then((groups) =>
        setMerchants(
  ...
  async function create(e: FormEvent) {
    e.preventDefault();
    await mutate("/member/cards", "POST", {
      ...form,
  ...
  async function remove(id: string) {
    if (confirm("確定刪除這張卡？已有交易的卡片將無法刪除。")) {
      try {
        await mutate(`/member/cards/${id}`, "DELETE");
        load();
      } catch (e) {
  ...
  const currentActivities = (c: import("./api").CatalogCard) =>
    c.activities.filter(
  ...
  return (
    <>
      ...
      <div className="card-list">
        {cards.map((c) => (
          <article
            className={`credit-card ${c.is_active ? "" : "inactive"}`}
            key={c.id}
          >
            <header>
              <span>{c.issuer}</span>
            ...
            <div>
              ...
              <small>
                {c.is_active ? "啟用" : "停用"}
                {c.account_tier && ` · ${c.account_tier}`} ·{" "}
                {c.last_four ? `•••• ${c.last_four}` : "未設定末四碼"}
              </small>
            </div>
            <button className="button danger" onClick={() => remove(c.id)}>
              <Trash2 size={15} />
              移除
            </button>
          </article>
          ...
          {catalog.map((c) => (
            ...
            >
              <div>
                ...
                <p>
                  {currentActivities(c).length
                    ? `${currentActivities(c).length} 個當前活動`
                    : "目前沒有有效活動"}
                </p>
              </div>
            </button>
      ...
      {detail && (
        ...
        >
          {currentActivities(detail).length > 0 ? (
            <ActivityDetails
              activities={currentActivities(detail)}
              units={units}
      ...
      {open && (
        <Dialog title="加入卡片夾" onClose={() => setOpen(false)}>
          <form className="stack" onSubmit={create}>
            <Field label="銀行卡片">
              <select
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    card_product_id: e.target.value,
                    account_tier: "",
                  })
                }
              >
            ...
            {selected && selected.account_tiers.length > 0 && (
              <Field label="目前帳戶等級">
                <select
                  required
                  value={form.account_tier}
                  onChange={(e) =>
                    setForm({ ...form, account_tier: e.target.value })
                  }
                >
                  <option value="">請選擇</option>
                  {selected.account_tiers.map((x) => (
            ...
            <Field label="卡號末四碼">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, last_four: e.target.value })
                }
              />
            </Field>
            ...
            <button className="button">加入卡片夾</button>
          </form>
        </Dialog>
      )}
    </>
...
function Transactions() {
  ...
  >([]);
  const load = () => api<typeof items>("/member/transactions").then(setItems);
  useEffect(() => {
    load();
  }, []);
  async function remove(id: string) {
    if (confirm("確定刪除這筆交易？回饋 cap 將會恢復。")) {
      await mutate(`/member/transactions/${id}`, "DELETE");
      load();
    }
  }
  return (
      ...
      <div className="list">
        {items.map((x) => (
          <article className="list-row" key={x.id}>
            <div>
              <h3>
                NT${(x.amount_minor / 100).toLocaleString()} ·{" "}
                {x.merchant_name || "其他店家"}
              </h3>
              <p>
                {x.category_code} · {x.payment_method_name} ·{" "}
                {x.transaction_date}
                {x.note && ` · ${x.note}`}
              </p>
            </div>
...
function MemberSettings() {
  ...
  function addTier() {
    ...
    setTiers((current) => [
      ...current,
      { id: `tier-${Date.now()}-${nextTierID.current++}`, units: [] },
    ]);
  }
  ...
  async function save() {
    ...
    setError("");
    try {
      await mutate("/member/reward-preferences", "PATCH", {
        preferences: toPreferenceWrites(tiers),
      });
  ...
  return (
    <>
      ...
      <div className="preference-actions">
        <button className="button" disabled={hasEmptyTier} onClick={save}>
          儲存偏好
        </button>
      </div>
    </>
