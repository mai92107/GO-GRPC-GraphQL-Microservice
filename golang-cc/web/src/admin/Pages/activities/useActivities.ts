...
export function useActivities() {
  ...
  const load = useCallback(async () => {
    try {
      const [nextCards, nextUnits, nextCategories, nextMethods, nextMerchants] =
        await Promise.all([
          getCards(),
          getRewardUnits(),
          getCategories(),
      ...
      setMethods(
        nextMethods.filter(
          (method) => method.is_active && method.id !== "any_payment",
        ),
      );
      ...
      setForm((current) => ({
        ...current,
        bank_name: current.bank_name || cardItems[0]?.bank_name || "",
        card_product_id: current.card_product_id || cardItems[0]?.id || "",
        networks: current.networks.length
          ? current.networks
          : (cardItems[0]?.networks || []).map((network) => network),
        reward_unit_id: current.reward_unit_id || nextUnits[0]?.id || "",
  ...
  const bankOptions = useMemo(
    () => [...new Set(cards.map((card) => card.bank_name))],
    [cards],
  );
  const availableCards = useMemo(
    () => cards.filter((card) => card.bank_name === form.bank_name),
    [cards, form.bank_name],
  );
  const selectedCard = cards.find((card) => card.id === form.card_product_id);
  const selectedNetworks = useMemo(
    () => (selectedCard?.networks || []).map((network) => network),
    [selectedCard],
  );
  const allNetworksSelected =
    selectedNetworks.length > 0 &&
    selectedNetworks.every((network) => form.networks.includes(network));
  ...
  const unavailableTypes = useMemo(
    () => unavailableSelectableTypes(items, benefits, form),
    [benefits, form, items],
  );
  const selectedCards = useMemo(
    () => filterBankCards(cards, activeBank, query, cardActivities),
    [activeBank, cardActivities, cards, query],
  );

  useEffect(() => {
    if (!showCreate || loading) return;
    if (!form.bank_name && bankOptions[0]) {
      setForm((current) => ({ ...current, bank_name: bankOptions[0] }));
      return;
    }
    if (
      !form.bank_name ||
      (form.card_product_id &&
        availableCards.some((card) => card.id === form.card_product_id))
    )
      return;
    setForm((current) => ({
      ...current,
      card_product_id: availableCards[0]?.id || "",
      network_ids: [],
      selectable_type: "",
    }));
  }, [
  ...
  useEffect(() => {
    ...
    const nextIDs = form.networks.filter((network) => allowed.has(network));
    if (!nextIDs.length && selectedNetworks.length)
      setForm((current) => ({ ...current, network_ids: selectedNetworks }));
    if (nextIDs.length && nextIDs.length !== form.networks.length)
      setForm((current) => ({ ...current, network_ids: nextIDs }));
  }, [form.networks, selectedCard, selectedNetworks, showCreate]);

  async function refreshCardActivities(cardID: string) {
    setCardLoadingID(cardID);
    setError("");
    try {
      const activities = mapCardActivities(await getCard(cardID));
      setCardActivities((current) => ({ ...current, [cardID]: activities }));
      setItems((current) => [
        ...current.filter((activity) => activity.card_product_id !== cardID),
        ...activities,
      ]);
      return activities;
    } catch (requestError) {
  ...
  async function toggleCard(card: CatalogCard) {
    if (expandedCards.includes(card.id)) {
      setExpandedCards((current) => current.filter((id) => id !== card.id));
      return;
    }
  ...
  function stageBenefit() {
    ...
    setError("");
    setBenefits((current) => [...current, benefitPayload(form)]);
    setForm((current) => ({
      ...current,
      benefit_name: "",
      monthly_cap: "",
  ...
  function selectBenefitMode(mode: BenefitMode) {
    setForm((current) =>
      nextFormForMode(current, mode, selectableTypes, unavailableTypes),
    );
  }
  ...
  async function create(event: FormEvent) {
    ...
    const allBenefits = form.benefit_name.trim()
      ? [...benefits, benefitPayload(form)]
      : benefits;
    if (!allBenefits.length) {
      setError("回饋方案至少需要一項優惠條件。");
      return;
    }
    setCreating(true);
    setError("");
    try {
      ...
      setExpandedCards((current) =>
        current.includes(form.card_product_id)
          ? current
          : [...current, form.card_product_id],
      );
    } catch (requestError) {
      setError((requestError as Error).message);
    } finally {
  ...
  async function toggle(activity: Activity) {
    ...
    try {
      ...
      await refreshCardActivities(activity.card_product_id);
    } catch (requestError) {
      setError((requestError as Error).message);
    } finally {
  ...
  function openRelationshipEditor(activity: Activity) {
    ...
    setEditBenefits(activity.benefits.map((benefit) => ({ ...benefit })));
    setError("");
  }
  ...
  async function saveRelationships(event: FormEvent) {
    ...
    setProcessingID(editingActivity.id);
    setError("");
    try {
      await updateActivity({
        ...editingActivity,
        benefits: editBenefits.map((benefit) => ({
          ...benefit,
          id: benefit.id || "",
          stack_group: benefit.stack_group.trim(),
        })),
      });
  ...
  async function scheduleVersion(benefit: Activity["benefits"][number]) {
    ...
    setError("");
    try {
      await publishRewardComponentVersion(benefit.id, {
        reward_unit_id: benefit.reward_unit_id,
        name: benefit.name,
        effect_type: benefit.effect_type,
        reward_value: rewardValue,
        effective_from: `${effectiveDate}T00:00:00+08:00`,
        effective_to: null,
        announced_at: new Date().toISOString(),
        change_reason: reason,
        display_change_until: `${effectiveDate}T23:59:59+08:00`,
      });
      const owner =
        editingActivity ||
        items.find((activity) =>
          activity.benefits.some((item) => item.id === benefit.id),
        );
